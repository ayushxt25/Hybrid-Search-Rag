...
class DocumentIndexingService:
    ...
    def index_document(
        ...
        embeddings = self.embedding_provider.embed_chunks(ingested_document.chunks)

        if len(embeddings) != ingested_document.chunk_count:
            raise RuntimeError("Embedding count does not match document chunk count.")

        self.vector_store.ensure_collection()

        indexed_points = self.vector_store.upsert_document(
